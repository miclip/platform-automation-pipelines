...
tempfile=$(mktemp)
tempproducts="/tmp/tempproduct.yml"
...
products=$(yq r vars-$opt_p/vars-products.yml 'products' | grep - | awk '{print $2}' )
for product_string in $products; do
    
    product_alias=$(echo $product_string | sed 's/\([^\|]*\)\|.*/\1/' )
    echo -e "\n#@data/values\n---\nproducts:\n- ${product_string}\n" >  $tempproducts
    ...
    if [ "$opt_s" == "true" ] ; then
        ...
        echo fly -t $opt_t set-pipeline -p $opt_n-$product_alias \
            -c $tempfile \
